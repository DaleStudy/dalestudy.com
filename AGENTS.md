...
- 섹션 제목은 `<h2>` 대신 `SectionHeading`을 쓰고, 감싸는 `<section>`에 같은 값의 `id`를 준다. 슬러그는 `src/anchors.ts`의 `ANCHORS`에 로케일과 무관한 영어로 적고 양쪽 모두 이 상수를 참조한다. 외부 링크가 의존하므로 한번 공개한 값은 바꾸지 않는다. CTA 배너 제목처럼 링크로 가리킬 이유가 없는 제목은 예외다
